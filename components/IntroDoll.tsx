...
import introDollFrames1024 from "./assets/intro-doll-frames-1024.webm";
import introDollFrames384 from "./assets/intro-doll-frames-384.webm";
...
// ffmpeg -framerate 1024 -pattern_type glob -i "intro-doll-frames/*.png" \
// -movflags faststart -vcodec libx264 -crf 23 -g 1 -pix_fmt yuv420p \
// intro-doll-frames.mp4
...
// ffmpeg -y -framerate 1024 -pattern_type glob -i "intro-doll-frames/*.png" \
// -c:v libvpx-vp9 -row-mt 1 -pix_fmt yuva420p \
// -b:v 0 -crf 52 -g 1 \
// intro-doll-frames-1024.webm

// ffmpeg -y -framerate 1024 -pattern_type glob -i "intro-doll-frames/*.png" \
// -c:v libvpx-vp9 -row-mt 1 -pix_fmt yuva420p -vf scale=384:384 \
// -b:v 0 -crf 42 -g 1 \
// intro-doll-frames-384.webm
...
// tar -cvf ../intro-doll-frames.tar *.webp
...
export default function IntroDoll(
	...
	const init = async (parent: HTMLDivElement, video: HTMLVideoElement) => {
		...
		// const tar = await (await fetch(introDollFrames)).arrayBuffer();
	...
	return (
		...
		>
			...
			>
				<source
					src={isMobile ? introDollFrames384 : introDollFrames1024}
					type="video/webm"
				></source>
			</video>
			<Flex
