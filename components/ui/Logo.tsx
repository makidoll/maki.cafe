...
export default function Logo() {
	...
	return (
		<svg
			...
			viewBox="0 0 630 380"
		>
			{["-shadow", ""].map((styleSuffix, i) => (
				<path
					key={i}
					className={[
						styles["m" + styleSuffix],
			...
			{["-shadow", ""].map((styleSuffix, i) => (
				<path
					key={i}
					className={[
						styles["i-line" + styleSuffix],
						styles["path" + styleSuffix],
					].join(" ")}
					d="M639.438,182.656l-0,112.5"
				></path>
			))}
			{["-shadow", ""].map((styleSuffix, i) => (
				<path
